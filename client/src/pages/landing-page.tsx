...
function SessionsMark({ size = 20, strokeWidth = 1.7, ringWidth = 1, pulseRadius = 0.95, halo = true, className = "" }: { size?: number; strokeWidth?: number; ringWidth?: number; pulseRadius?: number; halo?: boolean; className?: string }) {
  ...
  const arms: Array<{ x1: number; y1: number; x2: number; y2: number }> = [
  ...
  return (
    <svg viewBox="0 0 24 24" width={size} height={size} className={`text-foreground ${className}`} aria-label="Sessions" xmlns="http://www.w3.org/2000/svg">
      <defs>
        ...
        {arms.map((a, i) => (
          <linearGradient key={i} id={armId(i)} x1={a.x1} y1={a.y1} x2={a.x2} y2={a.y2} gradientUnits="userSpaceOnUse">
      ...
      {/* Outer ring with subtle gradient stop opacity via two arcs */}
      <circle cx="12" cy="12" r="10.25" fill="none" stroke="currentColor" strokeOpacity="0.22" strokeWidth={ringWidth} />
      <circle cx="12" cy="12" r="10.25" fill="none" stroke="#7dcfff" strokeOpacity="0.18" strokeWidth={ringWidth * 0.6} strokeDasharray="0.5 4" />

      {/* Arms — lit from center via per-arm gradient */}
      <g strokeWidth={strokeWidth} strokeLinecap="round">
        {arms.map((a, i) => (
          <line key={i} x1={a.x1} y1={a.y1} x2={a.x2} y2={a.y2} stroke={`url(#${armId(i)})`} />
        ))}
      </g>

      {/* Core node — luminous, soft outer halo + sharp inner highlight */}
      <circle cx="12" cy="12" r={pulseRadius * 2.4} fill="#7dcfff" opacity="0.35" filter={`url(#${glowId})`} />
      <circle cx="12" cy="12" r={pulseRadius * 1.7} fill={`url(#${coreId})`} />

      {/* Neurons firing inward — glow layer + sharp layer */}
      {tips.map((p, i) => (
        <g key={i}>
