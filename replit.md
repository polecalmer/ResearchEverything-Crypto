...
- **Self-learning query system:** The Data Agent uses a closed feedback loop inspired by autoresearch:
  ...
  - **Few-shot examples from proven queries:** When generating new charts, the system finds similar proven queries (same metric type, different protocol) and injects them as working examples in the LLM prompt. This dramatically improves first-try success because the LLM sees actual working SQL patterns instead of generating from scratch. The proven_queries table grows into a dynamic example bank — every successful query improves future queries for similar metrics.
