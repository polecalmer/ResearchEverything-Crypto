...
1.  **Server wallet → Anthropic (AI cost):** Server wallet (`0x8518b315b3DFC4415Be7E75b2571Df635b27552a`) pays Anthropic via mppx with Tempo payment method. Escrow channel maxDeposit: $5 (increased from $2 to accommodate 7-agent pipeline for liquid token projects). Real MPP costs are captured from the `onChallenge` callback (challenge.request.amount) and tracked per session.
