...
**Sessions (Conversational AI — unified research surface):** Renamed from "Research" to "Sessions" in all UI. The single entry point for all research, data, and analysis. Features a **Research/Data mode toggle** — Research mode for deep analysis, Data mode forces chart-building behavior (Sonnet model, deterministic pipeline, data-focused prompts). Charts are generated via a deterministic pipeline (`runChartPipeline()`) that bypasses the Opus agent loop: Sonnet extracts protocol + metric → registry lookup → DeFiLlama/CoinGecko fetch → server-side chart artifact + summary. The **Derived Metric Registry** (`server/data-source-brain/derived-metrics.ts`) defines `DerivedMetricRecipe` entries with declarative source bindings, trailing window configs, format types, and compute functions. Currently 8 metrics: pe_ratio, ps_ratio, take_rate, capital_efficiency, revenue_growth, fee_growth, volume_tvl_ratio, fdv_tvl. Falls back to proven queries → agent loop only if deterministic fetch fails. **Every chart is a saveable artifact** — InlineChart has a "Save" button that persists to `dashboard_charts` table. Saved charts appear in the "Charts" tab in the session sidebar. The sidebar now has three tabs: Sessions, Models, Charts. API endpoints: `POST /api/research/charts/save`, `GET /api/research/charts/saved`, `DELETE /api/research/charts/:id`. Full features: (1) highlight-to-dive-deeper, (2) Add to Reports, (3) Save as Model, (4) Save chart to dashboard, (5) inline charts/tables/metric cards, (6) 16 specialized tools including Dune SQL, DeFiLlama, CoinGecko, Allium, analyst corpus.
