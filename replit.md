...
**Session Research (Conversational AI):** A chat-based research interface at `/research` where users ask natural language questions about DeFi protocols and receive inline charts, tables, and analysis. Powered by a ReAct agent (`server/session-research-agent.ts`) with 9 tools (DefiLlama TVL/fees/revenue/volume/price/summary, list protocols, Dune SQL, Dune table discovery, compare protocols). Conversations are persisted with session management (create/list/delete). Frontend parses structured `artifact:chart` and `artifact:table` blocks from agent responses and renders them inline using Recharts. Routes: `/api/research/sessions` (CRUD), `/api/research/sessions/:id/messages` (chat). Uses `callAnthropicRaw` via MPP for AI inference.
