...
**Research Brain (Knowledge Graph):** An Obsidian-style persistent knowledge graph that accumulates intelligence across all research sessions. It stores typed entities, relationships, verified facts with provenance, and detects contradictions. Smart context retrieval uses entity-graph traversal and keyword matching to inject relevant prior knowledge into new session prompts. The frontend provides an interactive force-directed graph visualization.
