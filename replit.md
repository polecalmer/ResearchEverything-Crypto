...
**Session Research (Conversational AI):** A chat-based interface for researching DeFi protocols, providing inline charts, tables, metric cards, and analysis. Powered by a Claude Opus 4 ReAct agent with 16 specialized tools for data retrieval and analysis. Conversations are persisted with session management. Features include: (1) highlight-to-dive-deeper — select any section of assistant output and click "Dive Deeper" to recurse into that specific analysis, (2) "Add to Reports" button on each assistant message to save session output as a viewable report, (3) "Save as Model" button on messages with structured data (tables/charts/metrics) to extract and save as spreadsheet-style financial models, (4) professional presentation with properly sized typography (13px body, 14-18px headers), 300px-tall charts, card-based metric displays, and polished styling throughout. (5) **Chart Mode** — chart/data visualization requests (detected via regex `isChartRequest()`) use a **deterministic data-fetching pipeline** before handing to Opus: Sonnet classifies the metric type (P/E, revenue, TVL, volume, etc.), then `prefetchChartData()` fetches the actual data deterministically from DeFiLlama/CoinGecko APIs without any LLM. For P/E charts: fetches revenue + price + mcap/fdv/supply, computes all ratio variants (MCAP P/E, FDV P/E, Adj MCAP P/E) server-side, then gives Opus the pre-computed data to render in **1 round** (~8s). For revenue/fees/TVL/volume: fetches from DeFiLlama directly. Falls back to proven query injection if deterministic fetch fails. Opus stays as the render model for quality. Key code: `prefetchChartData()`, `searchProvenQueriesForProtocol()`, `buildProvenQueryFallback()` in `server/session-research-agent.ts`. Pattern learned from `executePeRatioProduction()` in `server/data-agent.ts`.
