...
**Data Agent (Library — folded into Sessions):** The Data Agent (`server/data-agent.ts`) is no longer a standalone feature — its chart/data capabilities are folded into Session Research. The standalone `/data` route redirects to `/research`. The Data Agent code remains as a library providing: Dune SQL execution patterns, proven query system, semantic coherence checking, data sanity validation, and self-learning from failures. The Data Brain view (proven queries, facts, learnings) remains accessible in the Brain page. The company-detail "Data" tab still uses the Data Agent directly for per-company chart generation.
