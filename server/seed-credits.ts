...
async function seedCreditProducts() {
  ...
  if (existing.data.length === 0) {
    const product10 = await stripe.products.create({
      name: '10 Credits',
      description: '10 deal enrichment credits for Sessions',
    ...
    const product50 = await stripe.products.create({
      name: '50 Credits',
      description: '50 deal enrichment credits for Sessions — best value',
  ...
  const subExisting = await stripe.products.search({ query: "name:'Sessions Pro Monthly'" });
  if (subExisting.data.length === 0) {
    const monthlyProduct = await stripe.products.create({
      name: 'Sessions Pro Monthly',
    ...
    console.log('Created: Sessions Pro Monthly ($20/mo) -', monthlyProduct.id);

    const yearlyProduct = await stripe.products.create({
      name: 'Sessions Pro Annual',
    ...
    console.log('Created: Sessions Pro Annual ($150/yr) -', yearlyProduct.id);
  } else {
