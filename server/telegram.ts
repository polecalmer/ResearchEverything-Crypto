...
export function startTelegramBot() {
  ...
  bot.command("start", async (ctx) => {
    ...
    await ctx.reply(
      "Welcome to Sessions Deal Bot!\n\n" +
      "To link your account:\n" +
      "1. Go to Sessions app → Settings\n" +
  ...
  bot.command("link", async (ctx) => {
    ...
    if (parts.length < 1) {
      await ctx.reply("Usage: /link YOUR_CODE\n\nGenerate a link code from your Sessions app first.");
      return;
    }
    ...
    if (!codeData) {
      await ctx.reply("Invalid or expired code. Generate a new one from your Sessions app.");
      return;
    }

    if (codeData.expiresAt < Date.now()) {
      linkCodes.delete(code);
      await ctx.reply("Code expired. Generate a new one from your Sessions app.");
      return;
    }
  ...
  bot.command("unlink", async (ctx) => {
    ...
    if (!user) {
      await ctx.reply("No account linked. Use /link to connect your Sessions account.");
      return;
    }
  ...
  bot.on("message:text", async (ctx) => {
    ...
    if (!user) {
      await ctx.reply(
        ...
        "Generate a link code from your Sessions app, then send: /link YOUR_CODE"
      );
      return;
    }
    ...
    if (credits <= 0) {
      await ctx.reply("No credits remaining. Purchase more credits in the Sessions app to continue using Telegram enrichment.");
      return;
    }
    ...
    try {
      ...
      if (enrichAvailable) {
        ...
        await ctx.api.editMessageText(
          ...
          `${company.name}\n` +
          `Open Sessions in your browser to run AI enrichment on this deal.\n\n` +
          `Added to: Discovered`
        );
      }
    } catch (error: any) {
