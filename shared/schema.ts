...
export const DATA_SOURCES = ["defillama", "coingecko", "dune", "allium", "stonksonchain", "exchanges"] as const;
...
/** Persisted record of a single backtest invocation. The plan column stores
 *  the BacktestPlan JSON; metrics + equityCurve hold the engine output. */
 ...
 *  Lets the agent reuse a vetted plan when the user re-asks a similar
 *  question, and surfaces few-shot examples to the NL→Plan translator. */
...
/** Health/staleness watermark for the WebSocket worker. The API surfaces
 *  `max(ts)` per (exchange, interval) to detect a stalled stream. */
